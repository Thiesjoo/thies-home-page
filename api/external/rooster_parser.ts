...
export default async function (req: VercelRequest, res: VercelResponse) {
	...
	if (process.env.VERCEL && (process.env.VERCEL_ENV === "production" || process.env.VERCEL_ENV === "preview")) {
		...
		rooster_data = (await axios({ url: "https://datanose.nl/195750.ics" }))?.data;
	} else {
		console.log("Caching rooster");
		//@ts-ignore
		rooster_data = readFileSync(join(process.env.ROOT, "rooster.ics"), "utf8");
	}

	const parsed = await ICAL.async.parseICS(rooster_data);
	res.setHeader("Cache-Control", "max-age=0, s-maxage=" + ((ms("7d") as number) / 1000).toString());
